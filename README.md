...
ברירת המחדל המקומפלת היא placeholder: `https://drafts-api.example.com` (ב‑`extension/src/config.ts`). יש שתי דרכים להצביע על השרת האמיתי שלכם:

- **בלי קומפילציה מחדש (מומלץ):** פתחו את ה‑popup של התוסף → "מתקדם" → הזינו את כתובת ה‑API שלכם. ה‑popup יבקש הרשאת host בזמן ריצה (`optional_host_permissions` כולל `https://*/*`), אז אשרו אותה.
- **בקומפילציה:** ערכו את `DEFAULT_API_URL` ב‑`extension/src/config.ts` ואת `host_permissions` ב‑`extension/manifest.json`, ואז הריצו שוב `npm run build:extension`.
...
1. ודאו שהשרת רץ ושכתובת ה‑API מוגדרת בתוסף (פרק 7).
2. פתחו את ה‑popup של התוסף.
3. לחצו על **"צור מפתח סנכרון חדש"** (Create Sync Key). אפשר לתת שם למכשיר (לדוגמה "מחשב בית").
4. השרת ייצר מפתח בפורמט `MTD-XXXX-XXXX-XXXX` (למשל `MTD-7Q4K-2H9P-X7P2`).
...
1. התקינו את התוסף במחשב השני (פרק 7) והגדירו בו את **אותה כתובת API**.
2. פתחו את ה‑popup → **"חבר מחשב קיים"** / **"התחברות עם מפתח סנכרון"** (Login).
3. הזינו את אותו ה‑**Sync Key** שיצרתם בפרק 8, ושם מכשיר (למשל "מחשב עבודה").
4. זהו — שני המחשבים חולקים כעת את אותו מפתח, וכל אחד מצפין/מפענח מקומית את אותן הטיוטות.
...
הצבת השרת מאחורי Nginx כ‑reverse proxy אל `127.0.0.1:3001`, והוצאת תעודת HTTPS עם certbot / Let's Encrypt עבור ה‑host (placeholder: `drafts-api.example.com`) — ראו:
